...
export default function GenerateLinkCode() {
  ...
  async function handleGenerate() {
    if (!user?.uid) { setMsg("Debes iniciar sesión."); return; }
    setBusy(true); setMsg("");
    try {
      const c = randomCode();
      ...
      await addDoc(collection(db, "invitaciones"), {
        ...
        code_hash: hash,
        created_at: serverTimestamp(),       // server timestamp OK
        expires_at: Timestamp.fromDate(expires), // explicit timestamp type
        valid: true,
        used: false,
        attempts: 0
      });
      ...
      setCode(c);
      setMsg("Código generado. Compártelo con tu terapeuta.");
    } catch (err) {
    ...
    <div className="mt-3">
      {code ? (
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { navigator.clipboard?.writeText(code); setMsg("Copiado!"); }} className="px-3 py-2 bg-indigo-600 text-white rounded">Copiar</button>
